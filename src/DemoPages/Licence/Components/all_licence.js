...
const AllLicence = () => {
    ...
    const SendQuotation = () => {
        var send_quotation = new Promise((resolve, reject)=>{
            ...
            try{
                licenceService.createQuotation(req).then(res=>{
                    if(res.success == true){
                        resolve("Our team will get back to you");
                    }else{
                        reject(res.error);
                    }
                }).catch(err=>{
        ...
        alertService.throwPromise(send_quotation);
    }
    ...
    return(
        <Fragment>
            ...
            <Modal toggle={(e)=>toggleModal()} size="lg" isOpen={modalOpen}>
                <ModalHeader className={style.customModalHeader} >Compare <img height="10" width="30" src = {logo} /> Plans</ModalHeader>
