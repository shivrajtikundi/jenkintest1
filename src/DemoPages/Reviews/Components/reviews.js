...
import { Row, Col, Card, CardBody, CardHeader, CardTitle, CardText, Button, CardFooter, Input,
    Modal, ModalHeader, ModalBody, ModalFooter, Form, FormGroup, Label, FormText, Progress,
    DropdownToggle, DropdownMenu,DropdownItem,UncontrolledButtonDropdown, Dropdown } from 'reactstrap';
...
const Reviews = () => {
    ...
    const MultiValue = props => {
        ...
        var crossbuttonImage = "https://cdn.imgbin.com/8/20/17/imgbin-computer-icons-button-check-mark-cross-red-cross-red-and-white-x-logo-V6upwmhHqRk2uDx6mmmqy7LdE.jpg";
        return <components.MultiValueLabel {...props}>{
            <div style={{padding:"4px"}}>
                <div className="multiSelectedData" style={{
    ...
    const getReviewSummaryOfApp = (e = false) =>{
        ...
        appService.getReviewSummaryOfAppFromPlayStore(req).then(res=>{
    ...
    const getReviewsOfAppPaginated = (e = false) =>{
        ...
        appService.getInhouseReviewsOfAppFromPlayStore(req).then(res=>{
            if(res.success == false){
                alertService.throwError(res.error);
                return;
            }else{
            ...
            if(res.success==true){
                ...
                res.data.forEach((elem, index)=>{
                    ...
                    var childjson = {
                        ...
                        icon: elem.app_icon,
                        store_icon : (elem.store_type == "ITUNES_APP_STORE")?"https://img.icons8.com/cute-clipart/2x/apple-app-store.png":"https://img.icons8.com/fluency/2x/google-play.png",
                        label:(elem.app_icon!="")?<div className={style.menuOption} data-multi-select-option="true" role="option" aria-selected="true" tabIndex="-1" >
                                                        <div style={{display:"flex"}}>
                                                            <div style={{display: "flex", flexDirection: "column", alignItems: "flex-start", justifyContent: "center", padding: "0px 7px"}}>
                                                            ...
                                                            <div style={{float:"left"}}  className={style.appNameDropDownMenuConatiner}>
                                                                <h4 className={cx(style.b_heading, style.b_heading__h4)}>
                                                                    {elem.app_title}
                                                                </h4>
                                                                <span className={cx(style.b_platform, style.b_platform__ios)}>
                                                                    {elem.store_type == "ITUNES_APP_STORE"?
                                                                    <><img height="16" width="16" alt="App Store Icon" src="https://img.icons8.com/cute-clipart/2x/apple-app-store.png"></img> App Store</>
                                                                    :
                                                                    <><img height="16" width="16" alt="Google Play icon" src="https://img.icons8.com/fluency/2x/google-play.png"></img> Google Play</>
                                                                    }
                                                                </span>
                                                            </div>
                                                        </div>
        ...
        <Fragment>
            <Row>
                <Col md="12">
                    ...
                    <p className={style.profleSubHeading}>View, search and get stats on reviews with text. <Link>Learn more</Link> →</p>
                </Col>
            </Row>
                    ...
                    <Card>
                        <CardHeader className={style.filterHeader}>
                            ...
                            <UncontrolledButtonDropdown className={style.filterHeaderDrpdwon}>
                                ...
                                <DropdownMenu  justified left className={cx("rm-pointers dropdown-menu-xs", style.customDropdownMenu)}>
                                    {source_list_array.map(function(elem,index){
                                        return(
                                            <DropdownItem toggle = {false} data-id = {elem._id} onClick={(e)=>manageSelection(e)} className={style.customSubMenu}>
                                                <div data-id = {elem._id} className={style.menuOption} data-multi-select-option="true" role="option" aria-selected="true" tabIndex="-1" >
                                                    
                                                    <div data-id = {elem._id} style={{display:"flex"}}>
                                                        <div data-id = {elem._id} className={style.selectionCheckboxContainer}>
                                                            <input data-id = {elem._id}  checked={(elem.is_checked == true)?true:false} name="585753" type="checkbox" className={style.selectionCheckbox}></input>
                                                        </div>
                                                        <div data-id = {elem._id} style={{display: "flex", flexDirection: "column", alignItems: "flex-start", justifyContent: "center", padding: "0px 7px"}}>
                                                            <div data-id = {elem._id} className={cx(style.b_appicon, style.b_appicon__google, style.b_appicon__medium)}>
                                                                <img style={{height: "35px", float:"left"}} src={elem.app_icon} />
                                                            </div>
                                                        </div>
                                                        <div data-id = {elem._id} style={{float:"left"}}  className={style.appNameDropDownMenuConatiner}>
                                        ...
                                        {all_stars.map(function(elem,index){
                                            return(
                                                <DropdownItem toggle = {false} className={style.customDropdownMenuforstars}>
                                                    <Input onClick={(e)=>onStarsSelection(e)} data-stars={elem} checked = {stars_selected.includes(elem)?true:false} type="checkbox" />{' '}
                                                    <StarRatings
                                                        rating={elem}
                                        ...
                                        {source_list_array.map(function(elem,index){
                                            return(
                                                <DropdownItem className={style.customSubMenu}>
                                                    <div className={style.menuOption} data-multi-select-option="true" role="option" aria-selected="true" tabIndex="-1" >
                                                        <div style={{display:"flex"}}>
                                                            <div style={{display: "flex", flexDirection: "column", alignItems: "flex-start", justifyContent: "center", padding: "0px 7px"}}>
                                                                <div className={cx(style.b_appicon, style.b_appicon__google, style.b_appicon__medium)}>
                                                                    <img style={{height: "35px", float:"left"}} src={elem.app_icon} />
                            ...
                            <div style={{float:"right"}}>
                                <Button variant="dark" onClick={()=>getReviews()}>Get Reviews</Button>
                            </div>
                        </CardHeader>
                    </Card>
                </Col>
                ...
                <Col md="12">
                    ...
                    <Card>
                        <CardHeader className={style.customCardHeader}>
                            ...
                            &nbsp;
                            <span className={style.headingDesc}>
                                Not matching the public store? You're probably looking for
                            </span>
                            &nbsp;
                            <Link className={style.customLink}>
                                Ratings
                            </Link>
                        </CardHeader>
                        <LoaderAdv
                                    ...
                                    <div className={style.reviewStatContainer}>
                                        ...
                                        <div className={style.reviewStatBody}>
                                            <div className={cx("widget-progress-wrapper", style.progessWidgetContainerForStarBifurcation)}>
                                                <Progress 
                                                    className={cx("progress-bar-sm progress-bar-animated-alt", style.customProgessBar, style.progessBarForStartBifurcation)}
                                                    color="warning"
                                                    value={
                                                        (five_star_reviews/(five_star_reviews + four_star_reviews + three_star_reviews + two_star_reviews + one_star_reviews))*100
                                                    }
                                                />
                                            ...
                                            <div className={cx("widget-progress-wrapper", style.progessWidgetContainerForStarBifurcation)}>
                                                <Progress 
                                                    ...
                                                    color="warning"
                                                    value={
                                                        (four_star_reviews/(five_star_reviews + four_star_reviews + three_star_reviews + two_star_reviews + one_star_reviews))*100
                                                    }
                                                />
                                                <div className={cx("progress-sub-label", style.customProgessSubLabel)}>
                                                    <div className={cx("sub-label-left", style.customSubLevelLeft)}>
                                            ...
                                            <div className={cx("widget-progress-wrapper", style.progessWidgetContainerForStarBifurcation)}>
                                                <Progress 
                                                    ...
                                                    color="warning"
                                                    value={
                                                        (three_star_reviews/(five_star_reviews + four_star_reviews + three_star_reviews + two_star_reviews + one_star_reviews))*100
                                                    }
                                                />
                                                <div className={cx("progress-sub-label", style.customProgessSubLabel)}>
                                                    <div className={cx("sub-label-left", style.customSubLevelLeft)}>
                                            ...
                                            <div className={cx("widget-progress-wrapper", style.progessWidgetContainerForStarBifurcation)}>
                                                <Progress 
                                                    ...
                                                    color="warning"
                                                    value={
                                                        (two_star_reviews/(five_star_reviews + four_star_reviews + three_star_reviews + two_star_reviews + one_star_reviews))*100
                                                    }
                                                />
                                                <div className={cx("progress-sub-label", style.customProgessSubLabel)}>
                                                    <div className={cx("sub-label-left", style.customSubLevelLeft)}>
                                            ...
                                            <div className={cx("widget-progress-wrapper", style.progessWidgetContainerForStarBifurcation)}>
                                                <Progress 
                                                    ...
                                                    color="warning"
                                                    value={
                                                        (one_star_reviews/(five_star_reviews + four_star_reviews + three_star_reviews + two_star_reviews + one_star_reviews))*100
                                                    }
                                                />
                                                <div className={cx("progress-sub-label", style.customProgessSubLabel)}>
                                                    <div className={cx("sub-label-left", style.customSubLevelLeft)}>
                    ...
                    <div 
                        ...
                        onScroll={onScroll}
                        ref={listInnerRef}>
                            {reviews_list && reviews_list.length > 0 && reviews_list.map(function(object, i){
                                // startLoading();
                                ...
                                // if(i >= loop_start_index && i <= loop_end_index){
                                    return (<div className={style.reviewWrapper}>
                                        <div className={style.reviewBody}>
                                            ...
                                            <h3 className={cx(style.bHeading, style.bHeadingH2, style.sTopMedium)} style={{whiteSpace: "pre-line", fontWeight: "600"}}>
                                                <span>{object.review_title && object.review_title!=null?object.review_title:""}</span>
                                            </h3>
                                            <p className={cx(style.bParagraph, style.tTable__rowGrow, style.sTopMedium)} style={{whiteSpace: "pre-line"}}>
                                            ...
                                            <div className={cx(style.review__actions, style.bTag)}>
                                                <div className={style.a__left}>
                                                    <div style={{display: "flex", alignItems: "center", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis"}}>
                                                        ...
                                                        <span>
                                                            <span className={cx(style.s_left__small, style.s_right__small)} style={{color: "rgb(220, 220, 220)"}}>•</span>
                                                            <a className={cx(style.b_link, style.b_link__light)} href="/reviews/2818609266_75940/?app=75940&amp;start=2022-02-24&amp;end=2022-05-25">
                                                                <svg width="16" height="16" viewBox="0 0 16 16" className={cx(style.iIcon, style.i_icon, style.i_icon__grey,style.i_icon__xsmall, style.s_right__small)}>
                                                                    <path d="M13.263 12.016c-.012-.128-.398-3.243-5.65-3.24v1.992a.346.346 0 01-.215.314.398.398 0 01-.398-.046L2.137 7.308A.337.337 0 012 7.04c0-.104.051-.203.138-.268l4.863-3.693a.4.4 0 01.397-.045c.131.058.215.18.215.313v1.949c2.179.064 3.878.712 4.972 1.897 1.78 1.928 1.383 4.73 1.365 4.846a.343.343 0 01-.348.295.333.333 0 01-.339-.317z" fillRule="evenodd"></path>
                                                                </svg>
                                                        ...
                                                        <span>
                                                            <span className={cx(style.s_left__small, style.s_right__small)} style={{color: "rgb(220, 220, 220)"}}>•</span>
                                                            <a className={cx(style.b_link, style.b_link__light)} href="/apps/75940-ola-the-top-ride-hailing-app/reviews/2818609266/external_reply">
                                                                <svg width="16" height="16" viewBox="0 0 16 16" className={cx(style.iIcon, style.i_icon, style.i_icon__grey,style.i_icon__xsmall, style.s_right__small)}>
                                                ...
                                                <div className={style.a_right}>
                                                    <div>
                                                        <span aria-label="Too expensive share menu" className="review-action" data-review-share-actions-callout="true">
                                                            <span className={cx(style.b_button, style.b_button__tiny, style.b_button__clean, style.b_tooltipped, style.b_tooltipped__top, style.b_tooltipped__nodelay)} aria-label="Options">
                                        ...
                                        <div className={style.review__meta}>
                                            <div className={style.meta_row}>
                                                <div style={{float:'left'}} className={style.meta__heading}>Author</div>
                                                <div style={{float:'left'}}>
                                                    <a className={cx(style.b_link, style.b_link__light)} href="https://www.google.com/search?q=+&quot;rawzeeee&quot; twitter OR facebook OR linkedin OR email OR @&quot;rawzeeee&quot;" style={{display: "flex", alignItems: "center"}}>
                                                        <span style={{whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "170px"}}>
                                                            <span>{object.review_user_name && object.review_user_name!=null?object.review_user_name:""}</span>
                                                        </span>
                                                        <div>
                                                            <svg width="16" height="16" viewBox="0 0 16 16" className={cx(style.iIcon, style.i_icon, style.i_icon__grey,style.i_icon__xsmall, style.s_right__small)}>
                                                                <path d="M14.17 2.698v4.564a.5.5 0 001 .026V1.513A.5.5 0 0014.657 1H8.824a.5.5 0 10.027 1h4.603l-8.32 8.137c-.162.163-.195.512 0 .707.196.196.545.163.708 0l8.328-8.146z"></path><path d="M2.5 4.5v9h9V7.016a.5.5 0 011 0V14a.5.5 0 01-.5.5H2a.5.5 0 01-.5-.5V4a.5.5 0 01.5-.5h6.98a.5.5 0 010 1H2.5z">
                                                                </path>
                                                            </svg>
                                                        </div>
                                                    </a>
                                            ...
                                            <div className={style.meta_row}>
                                                <div style={{float:'left'}} className={style.meta__heading}>Location</div>
                                                <div style={{float:'left'}}>
                                                    <span>India</span>
                                                </div>
                                            </div>
                                            <div className={style.meta_row}>
                                                <div style={{float:'left'}} className={style.meta__heading}>Language</div>
                                                <span style={{float:'left'}}>English</span>
                                            </div>
                                            <div className={style.meta_row}>
                                                <div style={{float:'left'}} className={style.meta__heading}>Version</div>
                                                <div style={{float:'left'}}>{object.review_on_app_version && object.review_on_app_version!=null?object.review_on_app_version:""}</div>
                                            </div>
                                            <div className={style.meta_row}>
                                                <div style={{float:'left'}} className={style.meta__heading}>Sentiment</div>
                                                <div style={{float:'left'}}>
                                                    <div style={{color: "rgb(230, 64, 58)", backgroundColor: "rgb(255, 241, 241)", padding: "0px 4px", borderRadius: "4px"}}>
                                                        Negative
                                                    </div>
                                                </div>
                                            </div>
                                            <div className={cx(style.meta__row, style.meta__tagwrapper)}>
                                                <div className={style.meta__heading} style={{paddingTop: "4px", float:"left"}}>Topics</div>
                                                <div className={cx(style.b_tag_wrapper, style.review_topics)}>
                                                    <div className={style.b_tag_wrapper}>
                                                        <a href="?topic=14" className={style.b_topic}>Connectivity</a>
                                                        <a href="?topic=18" className={style.b_topic}>Frequency</a>
                                                        <a href="?topic=19" className={style.b_topic}>Dissatisfied users</a>
                                                        <a href="?topic=30" className={style.b_topic}>Payment</a>
                                                        <a href="?topic=32" className={style.b_topic}>Privacy</a>
                                                    </div>
                                            ...
                                            <div className={cx(style.meta__row, style.meta__tagwrapper)} style={{paddingTop: "4px"}}>
                                                ...
                                                <div className={cx(style.b_tag_wrapper, style.review_topics)}>
                                                    <a href="/custom_topics/new" className={style.b_tag__button}>+ Create</a>
                                                </div>
                                            </div>
                                            <div className={cx(style.meta__row, style.meta__tagwrapper)}>
                                                <div className={style.meta__heading} style={{paddingTop: "7px"}}>Tags</div>
                                                <div className={cx(style.b_tag_wrapper, style.review_topics)}>
                                                    <div className={style.css_1o8xjgl_container}>
                                                        <span id="react-select-4-live-region" className={style.css_7pg0cj_a11yText}></span>
                                                        <span aria-live="polite" aria-atomic="false" aria-relevant="additions text" className={style.css_7pg0cj_a11yText}></span>
                                                        <div className={style.css_usw0b3_control}>
                                                            <div className={style.css_3ks0yi}>
                                                                <div className={style.css_9c3af7} data-value="">
                                                                    ...
                                                                    type="text" aria-autocomplete="list" aria-expanded="false" aria-haspopup="true" aria-controls="react-select-4-listbox" aria-owns="react-select-4-listbox" role="combobox" aria-describedby="react-select-4-placeholder" value="" 
                        ...
                        {loader == true && indexHolder != loop_end_index? 
                            <div style={{paddingLeft:"500px", paddingRight:"500px"}} className="font-icon-wrapper float-left mr-3 mb-3">
                                <div 
